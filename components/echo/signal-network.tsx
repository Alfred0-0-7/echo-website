...
export function SignalNetwork() {
  ...
  const activeCat = SIGNAL_CATEGORIES.find((c) => c.id === active) ?? null

  return (
    <section id="network" className="relative border-t border-white/5 py-16 sm:py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <div className="mb-14 text-center">
          <Reveal>
            <p className="mb-4 font-mono text-xs tracking-[0.35em] text-cyan">// COVERAGE</p>
          </Reveal>
          <Reveal delay={0.05}>
            <h2 className="font-display text-balance text-3xl font-bold tracking-tight sm:text-4xl md:text-5xl">
              THE SIGNAL <span className="text-cyan text-glow-cyan">NETWORK</span>
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mx-auto mt-5 max-w-xl text-pretty leading-relaxed text-muted-foreground">
              ECHO receives signals from every corner of the city. Hover a node to trace a
              ...
              {SIGNAL_CATEGORIES.map((c) => {
                ...
                return (
                  <line
                    ...
                    strokeDasharray={isActive ? '0' : '1 1.5'}
            ...
            {SIGNAL_CATEGORIES.map((c) => {
              ...
              return (
                ...
                >
                  <motion.span
                    animate={
                      isActive ? { scale: 1.25 } : reduce ? { scale: 1 } : { scale: [1, 1.08, 1] }
                    }
                    transition={
                      isActive
                        ? { duration: 0.2 }
